...
statusBarCmd= "dzen2 -bg '#2c2c32' -fg 'grey70' -w 620 -sa"

myManageHook = composeAll $
        [ className =? c --> doFloat | c <- myFloats ]
          where myFloats = ["feh", "Gimp", "MPlayer"]
...
main = do din <- spawnPipe statusBarCmd
          xmonad $ defaultConfig
                     ...
                     , workspaces         = ["1:dev", "2:mail", "3:web", "4:comm", "5:ham", "6:tmp", "7:tmp", "8:tmp", "9:tmp"] 
                     , terminal           = "urxvtcd"
                     , manageHook         = composeAll [manageDocks, myManageHook]
